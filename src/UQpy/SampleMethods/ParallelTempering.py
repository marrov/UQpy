...
class ParallelTemperingMCMC:
    ...
    :math:`p(x, T) \propto L(x)^{1/T} p_{0}(x)` for increasing values of the temperature T, where :math:`p_{0}` is
    a reference distribution (often set as the prior in a Bayesian setting) and :math:`L(x) = p(x)/p_{0}(x)`.
    Setting T=1 equates sampling from the target, while :math:`T \rightarrow \infty` samples from the reference
    distribution. Periodically during the run, the different temperatures swap members of their ensemble in a way that
    ...
    **Methods:**
    ...
    def __init__(self, niter_between_sweeps, factor_tempered=None, log_factor_tempered=None, args_factor_tempered=(),
                 log_pdf_reference=None, pdf_reference=None,
                 nburn=0, jump=1, dimension=None, seed=None, save_log_pdf=False, nsamples=None, nsamples_per_chain=None,
                 nchains=None, verbose=False, random_state=None, temperatures=None, ntemperatures=None,
                 mcmc_class=MH, **kwargs_mcmc):

        # MCMC specific inputs will be checked when instantiating MCMC objects
        ...
        self.ntemperatures = ntemperatures
        if self.temperatures is None:
            if self.ntemperatures is None:
                raise ValueError('UQpy: either input temperatures or ntemperatures should be provided.')
            elif not (isinstance(self.ntemperatures, int) and self.ntemperatures >= 2):
                raise ValueError('UQpy: input ntemperatures should be a integer >= 2.')
            else:
                self.temperatures = [np.sqrt(2) ** i for i in range(self.ntemperatures)]
        elif (not isinstance(self.temperatures, (list, tuple))
              or not (all(isinstance(t, (int, float)) and t >= 1. for t in self.temperatures))
              #or float(self.temperatures[0]) != 1.
        ):
            raise ValueError('UQpy: temperatures should be a list of floats, starting at 1. and increasing.')
        else:
            self.ntemperatures = len(self.temperatures)
        ...
        if not all((isinstance(val, (list, tuple)) and len(val) == self.ntemperatures)
        ...
        if isinstance(mcmc_class, MH) and len(kwargs_mcmc) == 0:
            ...
            kwargs_mcmc = {'proposal_is_symmetric': [True, ] * self.ntemperatures,
                           'proposal': [JointInd([Normal(scale=1. * np.sqrt(temp))] * dimension)
                                        for temp in self.temperatures]}

        # Initialize algorithm specific inputs: target pdfs
        self.ti_results = None
        self.evaluate_log_reference, _ = self._preprocess_target(
        ...
        for i, temp in enumerate(self.temperatures):
            self.mcmc_samplers.append(
                mcmc_class(log_pdf_target=self.evaluate_log_target[i],
        ...
        * **nsamples_per_chain** (`int`)
        ...
        while self.mcmc_samplers[0].nsamples_per_chain < final_ns_per_chain:
            ...
            if self.mcmc_samplers[0].niterations % self.niter_between_sweeps == 0:
                for i in range(self.ntemperatures - 1):
    ...
    def evaluate_evidence(self, compute_potential, log_p0=None, samples_p0=None):
        ...
        * **samples_p0** (`int`):
        ...
        beta_schedule = 1. / np.array(self.temperatures)
        log_pdf_averages = []
        for i, (temp, sampler) in enumerate(zip(self.temperatures, self.mcmc_samplers)):
            log_factor_values = sampler.log_pdf_values - self.evaluate_log_reference(sampler.samples)
            potential_values = compute_potential(
                x=sampler.samples, log_factor_tempered_values=log_factor_values, beta=1. / temp)
            log_pdf_averages.append(np.mean(potential_values))

        # use quadrature to integrate between 0 and 1
        from scipy.integrate import simps
        int_value = simps(x=beta_schedule[::-1], y=np.array(log_pdf_averages)[::-1])
        if log_p0 is None:
        ...
        self.ti_results = {
            'log_p0': log_p0, 'betas': beta_schedule[::-1], 'expect_potentials': np.array(log_pdf_averages)[::-1]}
